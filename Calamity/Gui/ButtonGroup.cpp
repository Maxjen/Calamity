...
Button::Button(SpriteManager* spriteManager, RenderLayer* renderLayer, wstring label, unsigned int fontId, Functor* functor) :
...
ButtonGroup::ButtonGroup(SpriteManager* spriteManager, unsigned int fontId, int width, bool fillHorizontal) :
	renderLayer(nullptr),
	spriteManager(spriteManager),
	minWidth(width),
	minHeight(0),
...
void ButtonGroup::setRenderLayer(RenderLayer* renderLayer) {
...
WidgetSize ButtonGroup::calculateMinSize() {
	WidgetSize result;
...
void ButtonGroup::setSize(int width, int height) {
	this->width = width;
	this->height = height;
}

WidgetSize ButtonGroup::getSize() {
	WidgetSize result;
	result.width = minWidth;
	result.height = minHeight;
	result.fillHorizontal = fillHorizontal;
...
void ButtonGroup::computeLayout() {
...
PriorityWidget ButtonGroup::getHighestPriorityWidget(int x, int y, SDL_Event* event) {
...
ActionHandler* ButtonGroup::createActionHandler(int x, int y, SDL_Event* event) {
...
void ButtonGroup::addButton(unsigned int row, wstring label, Functor* functor) {
	/*int top = 1 ? (row == 0) : 0;
